...
class ColorAnalyzer:
    ...
    @classmethod
    def analyze(cls, image_path: str, target_width_mm: float = 60.0, verbose: bool = True) -> ColorAnalysisResult:
        """
        分析图片，推荐最佳量化颜色数。
        ...
        # 默认结果
        default_result = ColorAnalysisResult(recommended=64, max_safe=128, unique_colors=0, complexity_score=50)
        ...
        try:
            ...
            if verbose:
                log.info(f"[ColorAnalysis] 分析尺寸: {w}x{h}, 像素数: {pixel_count:,}")
            ...
            if verbose:
                log.info(f"[ColorAnalysis] 宽度因子: {width_factor:.2f} (基于 {target_width_mm}mm)")
                total_time = time.time() - total_start
                log.info(f"[ColorAnalysis] ✅ 完成! 总耗时: {total_time:.2f}s")
                log.info(
                    f"[ColorAnalysis] 结果: 复杂度={complexity_score}, " f"推荐={recommended}, 最大安全={max_safe}"
                )
            ...
            log.exception(f"[ColorAnalysis] 分析失败: {e}")
            return default_result
    ...
    def _load_image(cls, image_path: str, verbose: bool):
        ...
        if verbose:
            log.info(f"[ColorAnalysis] 加载图片: {time.time() - t0:.2f}s, 原始尺寸: {w}x{h}")

        return img_rgb, w, h

    @classmethod
    def _resize_for_analysis(cls, img_rgb, original_w: int, target_width_mm: float, verbose: bool):
        """缩放图片到分析尺寸"""
    ...
    def _calc_unique_colors(cls, img_rgb, verbose: bool) -> int:
        ...
        if verbose:
            log.info(f"[ColorAnalysis] 独特颜色数（粗量化32级）: {unique_count}, " f"耗时: {time.time() - t0:.2f}s")
    ...
    def _calc_edge_complexity(cls, img_rgb, pixel_count: int, verbose: bool) -> float:
        ...
        if verbose:
            log.info(f"[ColorAnalysis] 边缘占比: {edge_ratio:.2%}, " f"耗时: {time.time() - t0:.2f}s")
