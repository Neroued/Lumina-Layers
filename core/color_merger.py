...
class ColorMerger:
    ...
    def identify_low_usage_colors(self, palette: List[dict], threshold_percent: float) -> List[str]:
        """
    ...
    def calculate_color_distance(self, color1_rgb: Tuple[int, int, int], color2_rgb: Tuple[int, int, int]) -> float:
        """
    ...
    def find_merge_target(
        self, source_color: str, palette: List[dict], max_distance: float, excluded_colors: Set[str]
    ) -> Optional[str]:
        """
        ...
        for entry in palette:
            ...
            if distance <= max_distance:
                candidates.append({"hex": hex_color, "distance": distance, "percentage": entry["percentage"]})
    ...
    def build_merge_map(self, palette: List[dict], threshold_percent: float, max_distance: float) -> Dict[str, str]:
        """
        ...
        if len(low_usage_colors) >= len(palette):
            log.warning("[COLOR_MERGER] All colors below threshold, merging disabled")
            return {}
        ...
        for source_hex in low_usage_colors:
            target_hex = self.find_merge_target(source_hex, palette, max_distance, excluded_colors)

            if target_hex is not None:
                merge_map[source_hex] = target_hex
            else:
                log.info(f"[COLOR_MERGER] No suitable target for {source_hex}, keeping original")

        return merge_map

    def apply_color_merging(self, matched_rgb: np.ndarray, merge_map: Dict[str, str]) -> np.ndarray:
        """
    ...
    def calculate_quality_metric(
        self, original_palette: List[dict], merged_palette: List[dict], merge_map: Dict[str, str]
    ) -> float:
        """
        ...
        for source_hex, target_hex in merge_map.items():
            ...
            # Calculate distance
            distance = self.calculate_color_distance(source_entry["color"], target_entry["color"])
