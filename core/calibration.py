...
from config import PrinterConfig, ColorSystem, SmartConfig, SmartConfigRYBW, MODELS_DIR, get_asset_path
...
def generate_smart_board_rybw(block_size_mm=5.0, gap_mm=0.8):
    ...
    corners = [(0, 0, 0), (0, total_dim - 1, 1), (total_dim - 1, total_dim - 1, 3), (total_dim - 1, 0, 2)]
    viewing_surface_z = 0
    for r, c, mat_id in corners:
        px = c * (pixels_per_block + pixels_gap)
        py = r * (pixels_per_block + pixels_gap)
    ...
    output_path = os.path.join(MODELS_DIR, generate_calibration_filename("6-Color", "SmartRYBW1296"))
    export_scene_with_bambu_metadata(
        scene=scene,
        output_path=output_path,
        slot_names=slot_names,
